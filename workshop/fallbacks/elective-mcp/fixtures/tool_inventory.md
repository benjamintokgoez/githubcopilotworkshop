...
| Filesystem and network confinement of that process (macOS and Linux) | `mcp.json`: `sandboxEnabled` on the server plus the top-level `sandbox` object |
| Which of the server's tools a chat session may pick | The client's tool picker and settings - **not** `mcp.json`, sandboxed or not |
| Whether you are asked to confirm each call | The client's approval settings - **unless** the server is sandboxed, in which case confirmations are auto-approved and your `sandbox` rules are the boundary instead |
...
Read that fourth row twice. Sandboxing is not confinement *plus* the prompts you
had before: it is confinement *instead of* them. `fixtures/config_notes.md` has
the reference wording and what it means for how you write the rules.
...
5. If you sandbox this server, every path and domain you allow becomes a standing
   grant with no per-call confirmation. Which of your current allowances would you
   still be comfortable with under that rule?
