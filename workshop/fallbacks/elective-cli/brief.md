...
| `staged_copy/permission_policy.md.txt` | Your current allow/ask/deny policy and evidence template |
...
2. For each event, record the required capability, verdict, current control, and
   blast radius.
3. Write at least three rules in the copied policy template. State the command
