...
- **Captured:** make verdicts before reading the transcript, map captured events
  to current controls, and trace one event through your completed policy. Label
  the result captured and `policy-traced`.
...
captured result with `Policy-traced:`; reserve `Observed live:` for a CLI request
that actually ran or prompted.
