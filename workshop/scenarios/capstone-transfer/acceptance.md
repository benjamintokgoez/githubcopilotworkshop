...
The verifier runs the staged standard-library suite from the scenario `work/`
directory. The starting state intentionally fails at multiple boundaries; save
the first run before changing code.
...
service and machine-payload boundary and a passing full verifier.
