"""API key lifecycle management for the MittelWerk platform.
...
``MITTELWERK_AUTH_SECRET_KEY`` environment variable. When neither is present a
...
SECRET_ENV_NAME = "MITTELWERK_AUTH_SECRET_KEY"  # noqa: S105
KEY_PREFIX = "mwk_"
...
#: ``dispatch`` guards work order submission/cancellation, ``admin`` is
#: reserved for key administration performed out of band.
VALID_PERMISSIONS = frozenset({"read", "dispatch", "admin"})
...
def _validate_organization_id(organization_id: str) -> str:
    ...
    normalised = organization_id.strip()
    if not normalised:
        raise ValueError("organization_id must be a non-blank string")
    return normalised
...
class APIKey:
    ...
    def __repr__(self) -> str:
        return (
            f"APIKey(key_id={self.key_id!r}, organization_id={self.organization_id!r}, "
...
class KeyManager:
    ...
    def _store(
        ...
        organization = _validate_organization_id(organization_id)
        ...
        logger.info(
            "Issued API key %s for organization %s (permissions=%s)",
            key_id,
            organization,
            sorted(perms),
        )
    ...
    def generate_key(
        ...
        self._store(raw_key, organization_id, permissions, ttl_seconds)
        return raw_key
    ...
    def register_key(
        ...
        return self._store(raw_key, organization_id, permissions, ttl_seconds)
    ...
    def revoke_key(self, key_id: str) -> bool:
        ...
        logger.info("Revoked API key %s for organization %s", key_id, record.organization_id)
        return True
    ...
    def list_keys(self, organization_id: str | None = None) -> list[APIKey]:
        """List key metadata — never secrets — optionally filtered by organization."""
        records = list(self._records.values())
        if organization_id is not None:
            wanted = _validate_organization_id(organization_id)
            records = [r for r in records if r.organization_id == wanted]
