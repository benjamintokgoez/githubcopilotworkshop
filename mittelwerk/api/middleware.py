...
delegated to :class:`~mittelwerk.auth.keys.KeyManager`, which performs a
timing-safe comparison against a keyed digest — the middleware never touches
private key storage. A missing header is ``401``; a key that is unknown,
expired, or revoked is ``403``.

Only the health endpoint, the interactive API docs, and the unauthenticated
dashboard shell are public. Everything that exposes dispatch, work order,
workload, or risk data requires a valid key.
"""
...
from mittelwerk.api.dependencies import API_KEY_HEADER, AuthPrincipal
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    Headers, query strings, and bodies are deliberately excluded so API keys
    and payload data can never reach the log.
    """
...
def configure_middleware(
    ...
    *,
    ...
    CORS is opt-in: with no configured origins the API stays same-origin,
    which is the safe default for a dashboard served from the same host. A
    wildcard origin is never combined with credentials. Malformed CORS
    configuration raises :class:`ValueError` here — before the application
    ever serves a request — instead of quietly authorising something
    unintended.
    ...
    response — including the ``401``/``403`` produced by authentication.
    Without that, a browser dashboard cannot read the auth failure at all and
    reports it as an opaque network error instead of "invalid key".
    """
